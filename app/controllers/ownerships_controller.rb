class OwnershipsController < ApplicationController
  ...
  def create
    ...
    if @item.new_record?
      begin
        response = Amazon::Ecs.item_lookup(params[:asin] , response_group: 'Medium' , country: 'jp')        # TODO 商品情報の取得 Amazon::Ecs.item_lookupを用いてください
      ...
      # TODO ユーザにwant or haveを設定する params[:type]の値ににHaveボタンが押された時には「Have」, Wantボタンがされた時には「Want」が設定されています
    if params[:type] == 'Have'
    ...
    # TODO 紐付けの解除  params[:type]の値ににHavedボタンが押された時には「Have」,Wantedボタンがされた時には「Want」が設定されています。 
  def destroy
      @item = Item.find(params[:item_id]) 
    if params[:type] == 'Have'
